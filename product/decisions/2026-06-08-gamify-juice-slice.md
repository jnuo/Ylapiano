...
| 13  | **"Works" = deliberate right-key, not raw hit-rate** (the windows let kids mash)               | Mei         | Generous ±600ms can reward mashing → must prove they learned note→key, not luck  |
| 14  | **Test age 5 first/hardest, ≥5 kids, local-only recording + signed consent**                   | Mei         | Design floor; COPPA: child face+voice is biometric; founder's own kid ≠ valid    |
...
- Playtest protocol → `docs/superpowers/specs/2026-06-08-playtest-protocol.md`

## Status

Committee design phase **complete** — scope (Anya), tech (Marco), hit-event
(Defne), motion (Diego), audio (Khalid), and the "does it work?" playtest
definition (Mei) all locked. Ready to build, with a measurable success target.
